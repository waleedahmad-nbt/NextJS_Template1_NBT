...
import "swiper/css";
import { getHeadphonesThumbnails } from "@/app/HeadphonesData";
...
const Details = () => {
  ...
  const selectedHeadphones = useSelector((state) => state.cart.selectedHeadphones);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedHeadphones.id)?.quantity ||
      0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedHeadphones.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedHeadphones.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedHeadphones));
  };

  const handleAddToFavorites = () => {
    dispatch(addToFavorites(selectedHeadphones));
  };
  ...
  const selectedHeadphonesId = selectedHeadphones ? selectedHeadphones.id : null;
  const thumbnails = getHeadphonesThumbnails(selectedHeadphonesId);
  ...
  const isDetailsPage = selectedHeadphones != null;

  const breadCrumbs = [
    { name: "Home", url: "/" },
    { name: "HeadPhones", url: "/pages/Headphones" },
    ...(isDetailsPage
      ? [
          {
            name: `${selectedHeadphones?.category}`,
            url: `/products/${selectedHeadphones?.category}`,
          },
          {
            name: `${selectedHeadphones?.title?.substring()} ...`,
            url: `/products/${selectedHeadphones?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedHeadphones.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedHeadphones)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  <div>
                    <p className="text-[#32BDE8] text-2xl font-semibold">
                      ${selectedHeadphones.originalPrice}
                    </p>
                  </div>
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedHeadphones.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
                ...
                <div className="flex flex-row gap-3 md:gap-8 items-center">
                  ...
                  <button
                    onClick={() => handleAddToCart(selectedHeadphones)}
