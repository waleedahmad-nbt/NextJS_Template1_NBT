...
import "swiper/css";
import { getElectronicsThumbnails } from "../ElectronicsData";
...
const Details = () => {
  ...
  const selectedElectronics = useSelector((state) => state.cart.selectedElectronics);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedElectronics.id)
        ?.quantity || 0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedElectronics.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedElectronics.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedElectronics));
  };
  ...
  const selectedElectronicsId = selectedElectronics ? selectedElectronics.id : null;
  const thumbnails = getElectronicsThumbnails(selectedElectronicsId);
  ...
  const isDetailsPage = selectedElectronics != null;

  const breadCrumbs = [
    { name: "Home", url: "/" },
    {name:"Computers, Tablets",url:"/pages/Compute&Tablet"},
    ...(isDetailsPage
      ? [
          {
            name: `${selectedElectronics?.category}`,
            url: `/products/${selectedElectronics?.category}`,
          },
          {
            name: `${selectedElectronics?.title?.substring()} ...`,
            url: `/products/${selectedElectronics?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedElectronics.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedElectronics)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  <div>
                    <p className="text-[#32BDE8] text-2xl font-semibold">
                      ${selectedElectronics.originalPrice}
                    </p>
                  </div>
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedElectronics.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
                ...
                <div className="flex flex-row gap-3 md:gap-8 items-center">
                  ...
                  <button
                    onClick={() => handleAddToCart(selectedElectronics)}
