...
import { addToCart, addToFavorites,setComputersTabletsDetails, setTvAudioDetails, setTvBoxDetails } from '@/app/lib/redux/slices/cartSlice';
import { TvandAudio } from '@/app/data3';
import { TvBox } from '@/app/TvboxData';
...
const ThreeColumn = () => {
    ...
    const handleTvBoxClick = (product) => {
        dispatch(setTvBoxDetails(product));
        router.push(`/pages/TvBoxDetails?${product.title}`);
    }
            ...
            <div className={`grid grid-cols-2 lg:grid-cols-3`}>
                {TvBox.map(product => (
                    <>
                        <div
                            key={product.id}
                            className='relative cursor-pointer h-[460px] w-full mt-6 p-1 md:p-3 gap-5 overflow-hidden 
                            border border-transparent hover:border-black rounded-lg'
                            onMouseEnter={() => setHoveredProduct(product.id)}
                            onMouseLeave={() => setHoveredProduct(null)}
                            onClick={()=>handleTvBoxClick(product)}
                        >
