...
import "swiper/css";
import { getTvBoxThumbnails } from "@/app/TvboxData";
...
const Details = () => {
  ...
  const selectedTvBox = useSelector((state) => state.cart.selectedTvBox);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedTvBox.id)?.quantity ||
      0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedTvBox.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedTvBox.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedTvBox));
  };

  const handleAddToFavorites = () => {
    dispatch(addToFavorites(selectedTvBox));
  };
  ...
  const selectedTvBoxId = selectedTvBox ? selectedTvBox.id : null;
  const thumbnails = getTvBoxThumbnails(selectedTvBoxId);
  ...
  const isDetailsPage = selectedTvBox != null;

  const breadCrumbs = [
    { name: "Home", url: "/" },
    { name: "Tv Box", url: "/pages/TVBox" },
    ...(isDetailsPage
      ? [
          {
            name: `${selectedTvBox?.category}`,
            url: `/products/${selectedTvBox?.category}`,
          },
          {
            name: `${selectedTvBox?.title?.substring()} ...`,
            url: `/products/${selectedTvBox?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedTvBox.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedTvBox)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  <div>
                    <p className="text-[#32BDE8] text-2xl font-semibold">
                      ${selectedTvBox.originalPrice}
                    </p>
                  </div>
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedTvBox.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
                ...
                <div className="flex flex-row gap-3 md:gap-8 items-center">
                  ...
                  <button
                    onClick={() => handleAddToCart(selectedTvBox)}
