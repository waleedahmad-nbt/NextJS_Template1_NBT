...
import "swiper/css";
import { getWatchThumbnails } from "../../SmartWatchData";
...
const Details = () => {
  ...
  const selectedWatch = useSelector((state) => state.cart.selectedWatch);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedWatch.id)?.quantity ||
      0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedWatch.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedWatch.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedWatch));
  };

  const handleAddToFavorites = () => {
    dispatch(addToFavorites(selectedWatch));
  };
  ...
  const selectedWatchId = selectedWatch ? selectedWatch.id : null;
  const thumbnails = getWatchThumbnails(selectedWatchId);
  ...
  const isDetailsPage = selectedWatch != null;
  ...
  const breadCrumbs = [
    ...
    ...(isDetailsPage
      ? [
          {
            name: `${selectedWatch?.category}`,
            url: `/products/${selectedWatch?.category}`,
          },
          {
            name: `${selectedWatch?.title?.substring()} ...`,
            url: `/products/${selectedWatch?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedWatch.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedWatch)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  ...
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedWatch.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
