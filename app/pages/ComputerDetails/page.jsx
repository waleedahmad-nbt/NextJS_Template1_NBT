...
import "swiper/css";
import { getComputerThumbnails } from "@/app/data2";
...
const Details = () => {
  ...
  const selectedComputer = useSelector((state) => state.cart.selectedComputer);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedComputer.id)
        ?.quantity || 0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedComputer.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedComputer.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedComputer));
  };
  ...
  const selectedComputerId = selectedComputer ? selectedComputer.id : null;
  const thumbnails = getComputerThumbnails(selectedComputerId);
  ...
  const isDetailsPage = selectedComputer != null;
  ...
  const breadCrumbs = [
    ...
    ...(isDetailsPage
      ? [
          {
            name: `${selectedComputer?.category}`,
            url: `/products/${selectedComputer?.category}`,
          },
          {
            name: `${selectedComputer?.title?.substring()} ...`,
            url: `/products/${selectedComputer?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedComputer.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedComputer)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  <div>
                    <p className="text-[#32BDE8] text-2xl font-semibold">
                      ${selectedComputer.originalPrice}
                    </p>
                  </div>
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedComputer.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
