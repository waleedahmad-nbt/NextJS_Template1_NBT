...
import "swiper/css";
import { getCellPhoneThumbnails } from "@/app/CellphoneData";
...
const Details = () => {
  ...
  const selectedCellPhone = useSelector((state) => state.cart.selectedCellPhone);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedCellPhone.id)
        ?.quantity || 0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedCellPhone.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedCellPhone.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedCellPhone));
  };
  ...
  const selectedCellPhoneId = selectedCellPhone ? selectedCellPhone.id : null;
  const thumbnails = getCellPhoneThumbnails(selectedCellPhoneId);
  ...
  const isDetailsPage = selectedCellPhone != null;

  const breadCrumbs = [
    { name: "Home", url: "/" },
    {name:"Cell Phone",url:"/pages/CellPhone"},
    ...(isDetailsPage
      ? [
          {
            name: `${selectedCellPhone?.category}`,
            url: `/products/${selectedCellPhone?.category}`,
          },
          {
            name: `${selectedCellPhone?.title?.substring()}`,
            url: `/products/${selectedCellPhone?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedCellPhone.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedCellPhone)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  <div>
                    <p className="text-[#32BDE8] text-2xl font-semibold">
                      ${selectedCellPhone.originalPrice}
                    </p>
                  </div>
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedCellPhone.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
                ...
                <div className="flex flex-row gap-3 md:gap-8 items-center">
                  ...
                  <button
                    onClick={() => handleAddToCart(selectedCellPhone)}
