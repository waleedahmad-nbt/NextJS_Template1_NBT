...
import "swiper/css";
import { getTvandAudioThumbnails } from "@/app/data3";
...
const Details = () => {
  ...
  const selectedAudio = useSelector((state) => state.cart.selectedAudio);
  const counter = useSelector(
    (state) =>
      state.cart.items.find((item) => item.id === selectedAudio.id)?.quantity ||
      0
  );

  const handleIncrement = () => {
    dispatch(increment({ itemId: selectedAudio.id }));
  };

  const handleDecrement = () => {
    dispatch(decrement({ itemId: selectedAudio.id }));
  };

  const handleAddToCart = () => {
    dispatch(addToCart(selectedAudio));
  };

  const handleAddToFavorites = () => {
    dispatch(addToFavorites(selectedAudio));
  };
  ...
  const thumbnails = getTvandAudioThumbnails(selectedAudioId);
  ...
  const isDetailsPage = selectedAudio != null;

  const breadCrumbs = [
    { name: "Home", url: "/" },
    { name: "Tv & Audio", url: "/pages/Tv&Audio" },
    ...(isDetailsPage
      ? [
          {
            name: `${selectedAudio?.category}`,
            url: `/products/${selectedAudio?.category}`,
          },
          {
            name: `${selectedAudio?.title?.substring()} ...`,
            url: `/products/${selectedAudio?._id}`,
          },
        ]
      : []),
  ];
              ...
              <div className="flex flex-col mt-6 md:w-[33%] lg:w-[40%]">
                <div className="flex flex-row ">
                  <h3 className="font-bold text-2xl md:text-3xl tracking-wid leading-10 text-start duration-700 ease-in-out text-black">
                    <a href="#">{selectedAudio.title}</a>
                  </h3>
                  <div
                    onClick={() => handleAddToFavorites(selectedAudio)}
                ...
                <div className="flex flex-row my-2 px-3 justify-between mt-6 font-thin">
                  <div>
                    <p className="text-[#32BDE8] text-2xl font-semibold">
                      ${selectedAudio.originalPrice}
                    </p>
                  </div>
                  <div className="flex flex-row items-center">
                    {Array.from({ length: selectedAudio.rating }).map(
                      (_, index) => (
                        <FaRegStar key={index} />
                      )
                    )}
                ...
                <div className="flex flex-row gap-3 md:gap-8 items-center">
                  ...
                  <button
                    onClick={() => handleAddToCart(selectedAudio)}
